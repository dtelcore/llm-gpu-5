...
def _parse_log_file(path: Path) -> Optional[RunSeries]:
    steps: List[int] = []
...
def _load_runs(paths: List[Path]) -> List[RunSeries]:
    runs = []
    for path in paths:
        if path.exists():
...
def print_summary(runs: List[RunSeries], smooth_window: int, forecast_window: int) -> None:
    ...
    print()
    print("┌" + "─" * (W - 2) + "┐")
    print("│  Training run summary" + " " * (W - 24) + "│")
    print("├" + "─" * (W - 2) + "┤")

    headers = f"  {'Run':<28}{'Loss':>8}{'ΔLoss':>8}{'PPL':>8}{'Forecast':>10}{'Tok/s':>8}{'Prog':>7}"
    print("│" + headers + " " * (W - 2 - len(headers)) + "│")
    print("├" + "─" * (W - 2) + "┤")
    ...
    for run in runs:
        ...
        fc_str = f"{fc['predicted_loss']:.4f}" if fc and fc["target_step"] > fc["observed_step"] else "—"
        ...
        loss_s  = f"{last_loss:.4f}"  if last_loss is not None else "—"
        delta_s = (("+" if delta >= 0 else "") + f"{delta:.4f}") if delta is not None else "—"
        ppl_s   = f"{last_ppl:.1f}" if last_ppl is not None else "—"
        toks_s  = f"{avg_toks:,.0f}"  if avg_toks is not None else "—"
        prog_s  = f"{progress:.1f}%"

        row = f"  {name:<28}{loss_s:>8}{delta_s:>8}{ppl_s:>8}{fc_str:>10}{toks_s:>8}{prog_s:>7}"
        print("│" + row + " " * max(0, W - 2 - len(row)) + "│")
...
def plot_runs_liveable(
    ...
    while True:
        current_runs = _load_runs(source_paths) if live else list(runs)
        if not current_runs:
